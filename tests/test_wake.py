...
class VerifyEnvironmentTests(WakeTestCase):
    ...
    def test_manifest_verifier_accepts_declared_core_layout(self):
        wake.write_core_manifest("Test")
        proc = self._run_verifier()
        self.assertEqual(proc.returncode, 0, proc.stdout + proc.stderr)
        report = json.loads(proc.stdout)
        self.assertEqual(report["status"], "STRUCTURALLY_COMPLETE")
        self.assertEqual(Path(report["memory_root"]).resolve(), self.memory.resolve())
        self.assertIn("identity", report["found_files"])
        self.assertIn("index", report["found_files"])
        self.assertIn("tool_runs", report["found_files"])

    def test_manifest_verifier_rejects_missing_declared_artifact(self):
        wake.write_core_manifest("Test")
    ...
    def test_manifest_verifier_fails_without_manifest(self):
        proc = self._run_verifier()
        self.assertEqual(proc.returncode, 1)
        report = json.loads(proc.stdout)
        self.assertEqual(report["status"], "STRUCTURALLY_INVALID")
        self.assertIn("core_manifest.json", report["mechanism"])
...
class EvidencePromotionTests(WakeTestCase):
    ...
    def test_successful_semantic_result_can_confirm_matching_hypothesis(self):
        ...
        notes = wake.apply_hypotheses_update(json.dumps({"status_change": [{
            "id": hid, "new_status": "confirmed",
            "evidence": "verify_environment.py tool-run reported STRUCTURALLY_COMPLETE",
            "conclusion": "confirmed"
        }]}), FIXED_NOW, journal)
