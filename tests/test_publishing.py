    ...
    def test_publish_is_isolated_and_repeatable(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            try:
                ...
                self.assertIn('data-nav="lab">Lab</a>', page)
                self.assertIn('stroke=%27%23286d72%27', page)
                for name in ("events.html", "state.html"):
                    readable = (project/"site"/name).read_text()
                    self.assertIn("wake-theme", readable)
                    self.assertIn("theme-toggle", readable)
                    self.assertIn("WAKE✳︎", readable)
                    self.assertIn("--paper:#f7f3ea", readable)
                    self.assertIn("--paper:#24283b", readable)
