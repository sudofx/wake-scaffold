...
WORKSPACE_DIR = MEMORY / "core_workspace"        # execution: tools, their run evidence, and the journal
...
def build_journal_prompt(reflection: str, now: datetime, enable_pull_requests: bool = False) -> str:
    ...
    return "\n\n---\n\n".join([
        ...
        "relevant, e.g. "
        '<a href="core_workspace/journal/2026-08-28-025254.md">that day</a>.\n\n'
    ...
    if not posts:
        ...
        for p in posts:
            ...
            if p.get("journal_entry"):
                journal_link = (f' — <a href="../../../core_workspace/journal/{p["journal_entry"]}">'
                                f'{p["journal_entry"]}</a>')
...
def write_daily_synthesis_index(now: datetime) -> Path:
    ...
    for entry_path in sorted(day_dir.glob("*.md")):
        ...
        entries.append(
            f"- [{entry_path.stem}](../../../../core_workspace/journal/{journal_name}): {excerpt}"
        )
