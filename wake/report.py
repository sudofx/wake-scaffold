...
def export(store, destination="site", experiment=None):
    with store.lock():
        state = store.load()
        events = store.events()
        _, head = store.replay()
        ...
        data = {"state": state, "events": events, "head": head, "generated": now(),
                "experiment": experiment, "timezone": "America/Los_Angeles"}
        ...
        page = page.replace("/* WAKE_SCRIPT */", (assets / "app.js").read_text()).replace("WAKE_DATA", embedded)
